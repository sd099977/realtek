 ...
 *  Function body
 ************************************************************************/
void _InitTimer(unsigned int dwInit)
{
#if 0 //def ROM_DEBUG
	prints("\n");
	prints(__FUNCTION__);
...
#define TIMERINFO_MSEC      1000000/37
	...
	_InitTimer(dwMiniSecond*TIMERINFO_MSEC);
	//Wait for Interrupt Pending Status
	while(_SystemTimeout() == FALSE);
}
...
int get_hwsetting(const int flash_type, const unsigned int target_addr)
{
	...
	switch (flash_type) {
		case SPI_FLASH_BOOT:
			res = get_hwsetting_spi(target_addr, HWSETTING_IN_CP42K_SRAM_SIZE);
			break;
#if 0
		case SPI_NAND_FLASH_BOOT:
			res = get_hwsetting_spi_n(target_addr, HWSETTING_IN_CP42K_SRAM_SIZE);
			break;
#endif
		case NAND_FLASH_BOOT:
			res = get_hwsetting_nand(target_addr, HWSETTING_IN_CP42K_SRAM_SIZE);
			break;
		case EMMC_BOOT:
			res = get_hwsetting_emmc(target_addr, HWSETTING_IN_CP42K_SRAM_SIZE);
			break;
...
int init_hwsetting_c(const int flash_type)
{
	...
	dst = HWSETTING_IN_CP42K_SRAM_ADDR;	
...
#if defined(DEBUG)
	hexdump("hwsetting", (void *)HWSETTING_IN_CP42K_SRAM_ADDR, HWSETTING_IN_CP42K_SRAM_SIZE);
#endif

	pre_hwsetting_c();

	//skip header
	res = parse_hw_setting_c((unsigned int *)(dst+4), HWSETTING_IN_CP42K_SRAM_SIZE, &count, 0);
