...
int get_nocs_certificate_emmc(unsigned int dst_addr)
{
	unsigned int blk_cnt = align_block_count(CERTIFICATE_SIZE);

	g_sys_flash_current_pos = EMMC_HW_SETTING_OFFSET / EMMC_BLOCK_SIZE;

	if( mmc_blk_read(g_sys_flash_current_pos, blk_cnt, (unsigned char *)dst_addr) )
		return ERR_GET_NOCS_CERTIFICATE_FAILED;

	g_sys_flash_current_pos += blk_cnt;

#if defined(DEBUG)
	hexdump("NOCS Certificate from eMMC", (char *)dst_addr, CERTIFICATE_SIZE);
	printf("g_sys_flash_current_pos: 0x%x\n", g_sys_flash_current_pos);
#endif
	return 0;
}

int get_nocs_params_emmc(unsigned int dst_addr)
{
	unsigned int blk_cnt = align_block_count(g_NOCS_Certificate.Size_Of_SCS_Params_Area);
...
#if defined(DEBUG)
	hexdump("Params Area from eMMC", (char *)dst_addr, g_NOCS_Certificate.Size_Of_SCS_Params_Area);
...
int get_nocs_aux_code_emmc(unsigned int dst_addr)
{
	unsigned int blk_cnt = align_block_count(g_Params_Area.Size_of_Aux_Code_Area);
...
int get_hwsetting_emmc(unsigned int dst_addr,  int size)
{
	...
	unsigned int blk_no = g_sys_flash_current_pos ;

	if( mmc_blk_read(blk_no, blk_cnt, (unsigned char *)dst_addr) )
...
{
	unsigned int blk_cnt = align_block_count(size);
	unsigned int blk_no = g_sys_flash_current_pos + align_block_count(SCS_UNCHECK_AREA);
	...
	while (blk_cnt > 0) {
		...
		if( mmc_blk_read(blk_no, read_cnt, (unsigned char *)dst_addr) )
