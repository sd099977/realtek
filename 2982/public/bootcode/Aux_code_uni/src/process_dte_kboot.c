...
static int verify_dte_kboot(const unsigned int img_addr, const int secure_type)
{
	...
	copy_memory(&g_dte_kboot_tail, (char *)(_img_addr + kboot_bin_size - 4), sizeof(DTE_KBOOT_AREA_Tail));
	...
	switch (secure_type) {
		...
		case DCAS_SECURE_BOOT:
			...
			copy_memory((char *)(tmp + kboot_bin_size), (char *)(_img_addr + kboot_bin_size), sizeof(DTE_KBOOT_AREA_Tail) - 4);
...
int process_dte_kboot(const int flash_type, const int secure_type, unsigned int *exist_kboot)
{
	...
	if (g_Params_Area.Address_of_Aux_Code_Area != 0)
	{
		g_dte_kboot_area_size = g_Params_Area.Address_of_Aux_Code_Area + sizeof(DTE_KBOOT_AREA_Tail);
