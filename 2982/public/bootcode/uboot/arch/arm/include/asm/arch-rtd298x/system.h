...
// this extra info is append to each kernel image in secure booting
// this needs to sync with the structuer image_file_creator uses
struct image_extra_info {
	unsigned int marker;
	unsigned int size;
...
#define GET_IC_VERSION()	rtd_inl(CRT_SC_VERID_VADDR)
#define VERSION_A			0x63300000
#define VERSION_B			0x63300001

// support 4 uarts, uart0 in ISO_MISC_OFF, uart1/2/3 in MISC
// this is configured by include/configs/xxx.h

// support 8 timers, timer0 for VCPU, timer1 for ACPU, timer2 for SCPU
...
// The definition of ddr address mapping in u-boot.
// Attention: if you need to modify the following settings,
//            please update the excel file in romcode/reference simultaneously.
...
#define MIPS_KCPU_FLAG_PTR_ADDR			(0x00000800)
...
//#define ECPU_KERNEL_ENTRY_ADDR		(0x00004000)	// 0x00004000~0x00009fff
#define ECPU_KERNEL_1_ENTRY_ADDR		(0x00004000)	// 0x00004000~0x00005fff
#define ECPU_KERNEL_2_ENTRY_ADDR		(0x00006000)	// 0x00006000~0x00007fff

#define EXTERN_PARAM_FROM_FLASH_ADDR	(0x0000a000)	// 0x0000a000~0x0000a7ff
#define ARM_REDIRECT_CODE2_ADDR			(0x0001f800)	// 0x0001f800~0x0001ffff
#define KCPU_CMDLINE_ADDR					(0x000fff00)	// 0x000fff00~0x00100000
#define KCPU_CMDLINE_SIZE					(0x00000100)	// 0x000fff00~0x00100000

#define BOOTCODE_TEXT_ADDR				(CONFIG_SYS_TEXT_BASE)
#define LINUX_KERNEL_ADDR				(0x00100000)
#define LINUX_KERNEL_ENTRY				(0x00108000)
#define ROM_HASH_RESULT_ADDR			(0x001FFF80)	// 128 byte for rom code hash result
...
#define MIPS_KCPU_FW_ENTRY_ADDR			(0x1a001000)
...
#define POWER_ON_VIDEO_BUFFER_SIZE		(0x04000000)	// 64M    //xq_xiao  for jpg log issue , suggest by suggest
#define POWER_ON_MUSIC_STREAM_ADDR		(0x06500000)	// 0650_0000~0670_0000
#define POWER_ON_MUSIC_STREAM_SIZE		(0x00200000)	// 2M
#define POWER_ON_VIDEO_STREAM_ADDR		(0x06700000)	// (0570_0000~0a00_0000)+16M (dynamic)
...
#define FACTORY_RO_DATA_ADDR   (0x0f400000) // 0f40_0000~0f80_0000 (4M)
#define TEMP_BUFFER_FOR_FLASH_DATA_ADDR (0x0f800000) // 0f80_0000~
...
#ifdef IS_298X_FPGA
#define ACPU_JUMP_ADDR_reg		(0xb801a618)	/* register which represent ACPU jump address (FPGA does not include ISO registers) */
#else
#define ACPU_JUMP_ADDR_reg		(0xb806012c)//(WDOG_DATA12)	/* register which represent ACPU jump address */
#endif

#define REG_CRT_SUSPEND_reg		(0xb8060100)//(WDOG_DATA1)	/* register which represent suspend to ram or disk */
#define REG_STR_JUMP_ADDR_reg	(0xb8060104)//(WDOG_DATA2)    /* register which represent STR jump address */
#define REG_KERNEL_RSV_MEM_reg	(0xb8060108)//(WDOG_DATA3)	/* register which represent kernel reserve memory or not */
#define REG_DDR_CAL_FLAG_reg	(0xb806010c)//(WDOG_DATA4)	/* register which represent DDR calibration status */
...
#define DC_SYS_CTRL_reg			(0xb8008690)
