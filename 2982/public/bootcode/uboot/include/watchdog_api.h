...
#define WATCHDOG_ENABLE()                             \
do {                                                                       \
    rtd_outl(MIS_TCWOV_reg, 0x08000000);            /*0.621377*8=4.97  s*/  \
    rtd_outl(MIS_TCWNMI_reg, 0x07ff0000);          \
    rtd_maskl(MIS_TCWCR_reg, ~0xFF, 0x00);       /*enable WDEN*/  \
} while(0)
...
#define WATCHDOG_REBOOT()                             \
    do {                                                                       \
		printf("WATCHDOG_REBOOT\n");										 \
        rtd_outl(MIS_TCWOV_reg, 0x08000000); /*IC  will be warm reseted after 0.3s elapsed*/  \
        rtd_outl(MIS_TCWTR_reg, 1); /*clear watchdog counter to prevent side-effect from current value*/   \
        rtd_maskl(MIS_TCWCR_reg, ~0xFF, 0x00);       /*enable WDEN*/  \
        while(1);       \
        /*void (*jump)(void) = (void *)Board_FLASH_ADDR_BASE;*/  \
        /*jump();*/  \
...
#else /* else of #ifdef CONFIG_REALTEK_WATCHDOG */
...
#endif /* endif of #ifdef CONFIG_REALTEK_WATCHDOG */
