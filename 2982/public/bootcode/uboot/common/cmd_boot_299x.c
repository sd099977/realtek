...
uint *VideoFlag_ptr_V1			= (uint *)MIPS_VIDEO_FLAG_PTR_V1_ADDR;		/* copy from darwin *streamfile_flagV */
uint *VideoFlag_ptr_V2			= (uint *)MIPS_VIDEO_FLAG_PTR_V2_ADDR;		/* rtd299x option */
...
uint *VideoFW_entry_ptr_V2		= (uint *)MIPS_VIDEO_FW_ENTRY_PTR_V2_ADDR;	/* rtd299x option */
...
#else
uint NAND_bootcode_area_size = SYS_PARAM_BOOTCODE_SIZE;		// nand bootcode area size
...
int checkHeader(UINT8* Buffer)
{
	...
	{
		printf("STD identifier NOT match, enter normal boot!!\n");
		return BOOTCODE_CONTINUE;
	}
...
static unsigned long do_go_video_fw(int vcpu_no)
{
	...
	switch (vcpu_no) {
		case 0:
			printf("Start Video Firmware 1/2 ...\n");
			mask_clock = (_BIT3 | _BIT4 | _BIT5 | _BIT7 | _BIT8 | _BIT9);
			mask_reset = (_BIT3 | _BIT4 | _BIT5 | _BIT7 | _BIT8 | _BIT9);

			*VideoFW_entry_ptr_V1 = SWAPEND32(MIPS_VIDEO_FW_1_ENTRY_ADDR | MIPS_KSEG1BASE);
			*VideoFW_entry_ptr_V2 = SWAPEND32(MIPS_VIDEO_FW_2_ENTRY_ADDR | MIPS_KSEG1BASE);
			break;

		case 1:
			printf("Start Video Firmware 1 ...\n");
			mask_clock = (_BIT3 | _BIT4 | _BIT5);
			mask_reset = (_BIT3 | _BIT4 | _BIT5);

			*VideoFW_entry_ptr_V1 = SWAPEND32(MIPS_VIDEO_FW_1_ENTRY_ADDR | MIPS_KSEG1BASE);
			break;

		case 2:
			printf("Start Video Firmware 2 ...\n");
			mask_clock = (_BIT7 | _BIT8 | _BIT9);
			mask_reset = (_BIT7 | _BIT8 | _BIT9);
	...
	{
		flush_dcache_all();
		enable_clock(SYS_SRST0_reg, mask_reset, SYS_CLKEN0_reg, mask_clock);
...
static unsigned long do_go_all_fw(void)
{
	...
	*VideoFW_entry_ptr_V2 = SWAPEND32(MIPS_VIDEO_FW_2_ENTRY_ADDR | MIPS_KSEG1BASE);
	...
	/* Enable VCPU1/VCPU2 */
	mask_clock = (_BIT3 | _BIT4 | _BIT5 | _BIT7 | _BIT8 | _BIT9);	// vcpu and ve
	mask_reset = (_BIT3 | _BIT4 | _BIT5 | _BIT7 | _BIT8 | _BIT9);
...
U_BOOT_CMD(
	...
	"\tv     - start video firmware 1/2\n"
	"\tv1    - start video firmware 1\n"
	"\tv2    - start video firmware 2\n"
	"\tk     - start kernel\n"
...
static int check_dvrboot_flag()
{
	...
	if ( usb_read_fat_file_size > MAX_USB_SCRIPT_LINE*MAX_USB_SCRIPT_LINE_LEN )
	{
		printf("[WARN] %s:read L4300_flashing.lst failed. (file size too large > %d)\n", __FUNCTION__, MAX_USB_SCRIPT_LINE*MAX_USB_SCRIPT_LINE_LEN);
		return 0;
	}
        for (i=0, j=0, endmark=0; i < usb_read_fat_file_size ; i++)
	{
			...
			{
				printf("[WARN] %s:read L4300_flashing.lst failed. (line length too large > %d)\n",__FUNCTION__, MAX_USB_SCRIPT_LINE_LEN);
				return 0;
			}
                	*(script+i) = '\0';
                	*(script+i+1) = '\0';
	...
	for (i=0; (i < (j-1)); i++)
	{
		...
		//printf("newline: %s\n", newline[i]);
		if(0 == strncmp("dvrboot.exe.bin", newline[i], 15))
                {
                        printf("dvrboot.exe.bin=%s\n", flagchar=skip_char(newline[i]+15, '='));
                        if (flagchar[0] == 'N')
				return 1;
			else
                                return 0;
...
int check_usb_tsbscript_update(const char *script_file_name, uchar bBootcode, uchar bUsb_need_init)
{
	uint target_addr = 0;
	uint data_bytes = 0;
	int ret = 0;
	...
	printf("tsbscript parms: bBootcode=%d, bUsb_need_init=%d, install_to_go_flag=%d\n", bBootcode, bUsb_need_init, install_to_go_flag);
        if (bUsb_need_init)
	{
		// usb start_ac
        ...
        if (bBootcode)
	{
		...
		else
		{
			...
			if(check_dvrboot_flag())
                        	return 1;  // dvrboot.exe.bin=N in L4300_flashing.lst
			else
				return 0;  // dvrboot.exe.bin=N Not in L4300_flashing.lst or marked by #
		}
        }
        else
        {
#ifdef CONFIG_USB_UPDATE_UBOOT_WHEN_AC_ON
		if (install_to_go_flag == 1)
		{
			/* Read success */
			return 1;
		}
		...
		{
			return 1;  // L4300_flashing.lst exits
		}
#endif
        }
}
...
int check_usb_update(const char *install_file_name, uchar bBootcode, uchar bUsb_need_init)
{
	...
	mcp_dscpt_addr = 0;
	secure_mode = ext_para_ptr->secure_mode;
	...
	else
	{
		...
		if ( ret > 0 )
		{
			...
			if( secure_mode == 0 ) {
				...
				reverse_signature( (unsigned char *)(target_addr+dst_length-rescue_truncated_size) );

				ret = Verify_SHA256_hash( (unsigned char *)target_addr,
				                          dst_length - rescue_truncated_size - 32 /* sha256 */,
				                          (unsigned char *)(target_addr + dst_length - rescue_truncated_size),
				                          1 );
			}
			if( ret < 0 ) {
...
int rtk_plat_run_rescue_from_NAND(void)
{
	...
	nand->read(nand, from, imageSize, &retlen, (unsigned int *)mem_layout.flash_to_ram_addr);
	if (retlen != imageSize)
	{
		/* Read error */
	...
	{
		/* Read success */
		printf("[OK] %s:Read escue linux  ok!!\n", __FUNCTION__);
	}
	...
	if( ret < 0 ) {
		printf("[ERR] %s: hash value not match\n", __FUNCTION__ );
		//return RTK_PLAT_ERR_READ_RESCUE_IMG;
		printf("[ERR] %s: bypass return error(Need to be fixed)\n", __FUNCTION__ );
	}
...
{
	uint	streamfile_size = 0;
//	uchar	*src;
//	char	rs[30];
	uint	lzma_src_addr;
//	char	*default_go_rescue = "go 0x80100000 ";
//	char	*endp;
//	unsigned long num = 0;
...
//	initmodulesEx2();
	...
	if (ext_para_ptr->rescue_img_size == 0)
	{
		printf("[ERR] Rescue image size error!!\n");
		return RTK_PLAT_ERR_READ_RESCUE_IMG;
	}

	if (ext_para_ptr->rescue_img_part0_saddr == 0)
	{
		printf("[ERR] Rescue first part address error!!\n");
		return RTK_PLAT_ERR_READ_RESCUE_IMG;
	}

	if (ext_para_ptr->rescue_img_part0_len == 0)
	{
		printf("[ERR] Rescue first part length error!!\n");
		return RTK_PLAT_ERR_READ_RESCUE_IMG;
	}
	...
	{
		printf("[ERR] Rescue total size incompatible error!!\n");
		return RTK_PLAT_ERR_READ_RESCUE_IMG;
	}
...
//	src = ext_para_ptr->rescue_img_part0_saddr;
	...
	{
		memcpy((char *)(rtk_mem_info->encrypted_fw_image_addr + ext_para_ptr->rescue_img_part0_len),
			   ext_para_ptr->rescue_img_part1_saddr,
			   ext_para_ptr->rescue_img_part1_len);

		printf("copy rescue1: from %x to %x, len = %x\n",
			(UINT32)ext_para_ptr->rescue_img_part1_saddr,
			rtk_mem_info->encrypted_fw_image_addr + ext_para_ptr->rescue_img_part0_len,
			ext_para_ptr->rescue_img_part1_len);
	}

#if 0 // mark secure
	if (check_secure_boot() == SECURE_BOOT) {
		unsigned int * Kh_key_ptr = DEFAULT_KEY_PTR;
		lzma_src_addr = 0x83000000;
		// decrypt image
		if (decrypt_image((char*)COPY_MERGE_ADDRESS,
			(char *)lzma_src_addr,
			streamfile_size,
			Kh_key_ptr)) {
			printf("decrypt rescue error!\n");
			return RTK_PLAT_ERR_READ_RESCUE_IMG;
		}
	}
	else
		lzma_src_addr = rtk_mem_info->encrypted_fw_image_addr;
#else
	lzma_src_addr = rtk_mem_info->encrypted_fw_image_addr;
#endif
...
#if 0
	if( Lzma_main((char *)lzma_src_addr,
				  (char *)UNZIP_ADDRESS 	,streamfile_size, rs, NULL) != 0)
	{
		printf("%s\n", rs);
	...
	if (lzmaBuffToBuffDecompress((uchar*)LINUX_KERNEL_ENTRY, &uncompressedSize, (uchar*)lzma_src_addr, streamfile_size) != 0)
	{
		printf("[ERR] decompess Rescue using LZMA error!!\n");
...
//	sys_flush_caches();
	...
	/* init go_rescue string to default value */
//	memset(go_rescue, 0, sizeof(go_rescue));
//	strncat(go_rescue, default_go_rescue, strlen(default_go_rescue));

#if 0 // mark rescue parameter
	/* check if rescue_param has value */
	memset(rs, 0, sizeof(rs));
	if (memcmp(rs, rescue_param, sizeof(rescue_param))) {
		// check if rescue_param contains address value
		num = strtoul(rescue_param, &endp, 16);
		if (num != 0) {
			sprintf(rs, "BOOT_TAG=");
			strncat(rs, rescue_param, sizeof(rescue_param));
			strncat(go_rescue, rs, strlen(rs));
		}
	}
#endif
	...
	if (env_split_and_save("RSA_KEY_MODULUS", Config_Secure_RSA_MODULUS, 96) != OK) {
			printf("set env %s failed\n", "RSA_KEY_MODULUS");
		return RTK_PLAT_ERR_READ_RESCUE_IMG;
	}
#endif

#if defined(Config_Secure_AES_TRUE)
	// set AES img key to env variable
		if( env_set( "AES_IMG_KEY", Config_Secure_AES_IMG_KEY_STR, ENV_ATTR_RW, Config_Secure_AES_IMG_KEY_STR, NULL ) != OK ) {
			printf("set env %s failed\n", "AES_IMG_KEY");
			return RTK_PLAT_ERR_READ_RESCUE_IMG;
		}
#endif
...
//////////////////////////////////////////////////////////////////////////
	...
	else
	{
		...
		*VideoFW_entry_ptr_V2 = CPU_TO_BE32(target_addr);
...
//	uint target_addr;
	...
	if(!g_isSTR)
	{
		...
		//setup default value
		boot_av->audio_buffer_addr	= SWAPEND32(0x85000000);
		boot_av->audio_buffer_size	= SWAPEND32(0x00100000); // 1M
		boot_av->video_buffer_addr	= SWAPEND32(0x86000000);
		boot_av->video_buffer_size	= SWAPEND32(0x02000000); // 32M
		boot_av->last_image_addr	= SWAPEND32(0x01300000);
		boot_av->last_image_size	= SWAPEND32(0x003B6C00);
	}
	/* parse each fw_entry */
		...
		if (this_entry->target_addr)
		{
			switch(this_entry->type)
			{
				...
				case FW_TYPE_VIDEO_2:
					*VideoFW_entry_ptr_V2 = CPU_TO_BE32(this_entry->target_addr);
					printf("Video FW 2:\n");
					selV1_V2|=0x02;
					break;
				...
				case FW_TYPE_AUDIO_FILE:
					continue;
					this_entry->target_addr = 0x83000000;
					audioFileNum++;
					...
					if (boot_music_enable/* && boot_power_on_mode*/)
					{
						printf("Audio data %d:\n", audioFileNum);
						...
						{
							boot_av->dwAudioStreamLength = custom_audio1_length;
						}
						else if (boot_music_enable==3 && custom_audio2_length != 0)
						{
							boot_av->dwAudioStreamLength = custom_audio2_length;
						}
						else
#endif
                                        {
				...
				case FW_TYPE_VIDEO_FILE:
					continue;
					this_entry->target_addr = 0x83100000;
					videoFileNum++;
					...
					if (boot_logo_enable/* && boot_power_on_mode*/)
					{
						printf("Video data %d:\n", videoFileNum);
						...
						if(default_panel_parameter.iVFLIP)
							boot_av->bRotate = 1;
						else
							boot_av->bRotate = 0;
						...
						{
							boot_av->dwVideoStreamLength = SWAPEND32(custom_video1_length);
							this_entry->length = custom_video1_length;
						}
						else if (boot_logo_enable == 3 && custom_video2_length != 0)
						{
							boot_av->dwVideoStreamLength =	SWAPEND32(custom_video2_length);
							this_entry->length = custom_video2_length;
						}
						else
#endif
						{
...
#if 1 // refine read fw flow with secure and lzma option
			if (this_entry->type != FW_TYPE_KERNEL/* && boot_power_on_mode*/)
			{
			...
			if(this_entry->lzma)
			{
				...
				if ( (check_secure_boot() == SECURE_BOOT) &&
					(this_entry->type == FW_TYPE_KERNEL ||
					 ...
					 this_entry->type == FW_TYPE_VIDEO) ) {
					display_evaluate_time("before \"decrypt_image\"");
					// decrypt image
					if (decrypt_image((char*)fw_desc_table_base + this_entry->offset,
						(char *)COPY_MERGE_ADDRESS,
						this_entry->length,
						Kh_key_ptr)) {
						printf("decrypt image(%d) error!\n", this_entry->type);
						return NULL;
					}
					display_evaluate_time("after \"decrypt_image\"");
					src = (char *)COPY_MERGE_ADDRESS;
					size = this_entry->length;

					// verify RSA signature if needed
					if (version == FW_DESC_TABLE_V1_T_VERSION_21) {
#if defined(Config_Secure_RSA_TRUE)
						...
						display_evaluate_time("before \"rsa_verify\"");
						rsa_verify(signature, Config_Secure_RSA_MODULUS, checksum);
						display_evaluate_time("after \"rsa_verify\"");

						// calculate SHA1 hash value of decrypted image
						memset(sha1_hash, 0, sizeof(sha1_hash));
						if (do_sha1((uchar *)(this_entry->target_addr), this_entry->length, sha1_hash)) {
							printf("do_sha1 failed\n");
							return NULL;
						}

						hash_str = bin2str(sha1_hash, SHA1_SIZE);
						//printf("recovered hash: %s\n", checksum);
						//printf("calculated SHA1 hash:%s\n", hash_str);

						// compare recovered hash value with calculated hash value
						if (memcmp(hash_str, checksum, SHA1_SIZE)) {
							printf("fw hash value not match!\n");
...
#else
						printf("no RSA key pre-defined in bootcode!\n");
						return NULL;
...
#endif
				{
					src = (char *)(fw_desc_table_base + this_entry->offset);
					size = this_entry->length;
				}

				uint uncompressedSize;
				if (lzmaBuffToBuffDecompress((uchar*)this_entry->target_addr, &uncompressedSize, (uchar*)src, size) != 0)
				{
					printf("Decompress using LZMA error!!\n");
				...
				if (this_entry->type != FW_TYPE_KERNEL && boot_power_on_mode)
				{
					...
					if ( (secure_mode != NONE_SECURE_BOOT) &&
						(this_entry->type == FW_TYPE_AUDIO ||
							...
							this_entry->type == FW_TYPE_ECPU) )
					{
						source_addr = rtk_mem_info->encrypted_fw_image_addr - (this_entry->offset % EMMC_BLOCK_SIZE);
					}
					else
					{
						source_addr = this_entry->target_addr - (this_entry->offset % EMMC_BLOCK_SIZE);
					}

					// get the physical address
					source_addr &= (~MIPS_KSEG_MSK);

					if (!rtk_eMMC_read(block_no, this_entry->length, (uint *)source_addr))
					{
						printf("[ERR] Read fw error (type:%d)!\n", this_entry->type);
					...
					fw_checksum = get_checksum((uchar *)source_addr, this_entry->length);

					if (this_entry->checksum == fw_checksum)
					{
						;//printf("\t FW checksum PASS (0x%08x)\n", fw_checksum);
					}
					else
					{
						printf("\t FW Image checksum FAILED\n");
					...
					if ( (secure_mode == RTK_SECURE_BOOT) &&
						(this_entry->type == FW_TYPE_AUDIO ||
							...
							this_entry->type == FW_TYPE_ECPU) )
					{
						rtk_hexdump("the first 32-byte encrypted data", (unsigned char *)rtk_mem_info->encrypted_fw_image_addr, 32);

						// decrypt image
						printf("to decrypt...\n");
						target_addr = this_entry->target_addr & (~MIPS_KSEG_MSK);
						if (decrypt_image((char *)rtk_mem_info->encrypted_fw_image_addr,
							(char *)target_addr,
							this_entry->length,
							Kh_key_ptr))
						{
						...
						reverse_signature( (unsigned char *)(target_addr+this_entry->length-img_truncated_size) );

						ret = Verify_SHA256_hash( (unsigned char *)target_addr,
						                          this_entry->length - img_truncated_size - 32 /* sha256 */,
						                         (unsigned char *)(target_addr + this_entry->length - img_truncated_size),
						                          1 );
						if( ret < 0 ) {
							printf("[ERR] %s: verify hash fail(%d)\n", __FUNCTION__, ret );
							return RTK_PLAT_ERR_READ_FW_IMG;
...
//				strncat(str_phash, buf, strlen(buf));
	...
	if(!g_isSTR)
	{
		...
		if (boot_av->dwMagicNumber == SWAPEND32(BOOT_AV_INFO_MAGICNO))
		{
			...
			//printf("logo output mode: %s\n", sys_logo_is_HDMI ? "HDMI": "DVI");

			// enable audio sound
			if (boot_av->dwAudioStreamLength != 0)
			{
#if defined(Board_Chip_Rev_1185)
							/* hcy : need modify in Saturn	*/
				REG32(KSEG1(ISO_GPDATO)) |= 0x100;	// ISO GPIO 8
#elif defined(Board_Chip_Rev_1055)
				REG32(KSEG1(ISO_GPDATO)) |= 0x10;	// ISO GPIO 4
#endif
			}
...
int rtk_plat_read_fw_desc_table_from_NAND(void)
{
	...
	NAND_fw_desc_table_start = NAND_bootcode_area_size + CONFIG_FACTORY_SIZE;
	fw_desc_table_base = rtk_mem_info->fw_desc_table_base_addr;
...
int rtk_plat_boot_setup(bootm_headers_t *images)
{
	...
	if (boot_from_flash == BOOT_FROM_FLASH_NORMAL_MODE)
	{
		if (ret == RTK_PLAT_ERR_OK)
		{
			...
			{
				printf("Enable VCPU1\n");
				mask_clock |= (_BIT3 | _BIT4 | _BIT5);	// vcpu 1 and ve 1
				mask_reset |= (_BIT3 | _BIT4 | _BIT5);
			}

			if (*VideoFW_entry_ptr_V2 != 0)
			{
				printf("Enable VCPU2\n");
				mask_clock |= (_BIT7 | _BIT8 | _BIT9);	// vcpu 2 and ve 2
				mask_reset |= (_BIT7 | _BIT8 | _BIT9);
			}

			if (*AudioFW_entry_ptr != 0 && (*VideoFW_entry_ptr_V1 != 0 || *VideoFW_entry_ptr_V2 != 0))
			{

//				unsigned int acpu_tmp1 = rtd_inl(0x00000094);
//				unsigned int acpu_tmp2 = 0;
//				unsigned int iii = 0;

				printf("aa 0x%x, 0x%x, 0x%x, 0x%x\n", rtd_inl(0x00000094), rtd_inl(0x00000098), rtd_inl(0x0000009c), rtd_inl(0x000000a0));

#ifndef ENABLE_DDR_CALIBRATION
...
#if 0
				acpu_tmp2 = rtd_inl(0x00000094);

				while (acpu_tmp2 == acpu_tmp1) {
					iii++;
					printf("\033[1;31m Wait Release HW semaphore for acpu not run, 0x%08x, (%d)\033[m\n", acpu_tmp2, iii);
					release_hw_semaphore();
					WATCHDOG_KICK();
					...
					acpu_tmp2 = rtd_inl(0x00000094);
					printf("aa(%d) 0x%x, 0x%x, 0x%x, 0x%x\n", iii, rtd_inl(0x00000094), rtd_inl(0x00000098), rtd_inl(0x0000009c), rtd_inl(0x000000a0));
