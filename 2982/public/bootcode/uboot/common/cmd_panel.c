...
int panelspec_parse_main(char* panelspec_file_name_full_path)
{
	...
	if (tar_read(panelspec_file_name_full_path, (char *)rtk_mem_info->factory_data_addr, CONFIG_FACTORY_SIZE/2, &dst_addr, &dst_length) > 0)
	{
		// parse header files
...
void panel_dump_panel_bin_list(void)
{
	...
	tar_read("DUMMY_NAME_FOR_DUMP", (char *)rtk_mem_info->factory_data_addr, CONFIG_FACTORY_SIZE/2, &dst_addr, (uint *)&dst_length);
...
#else
#if 0
	if (tar_read(PANEL_SPEC_INFO_FILE_NAME, (char *)rtk_mem_info->factory_data_addr, CONFIG_FACTORY_SIZE/2, &dst_addr, &dst_length) != NULL)
	{
