...
#include <common.h>
#include <command.h>
#ifdef CONFIG_REALTEK_EERPOM_TV010_A
...
#else
	printf("this board no support amp ...\n");
#endif
	return 0;
}
	...
	if (hex_string == NULL) {
		switch(eep_address_index) {
			...
			case 'd':
				...
				break;
			case 'e':
				Eeprom_Read(EEP_AD_PANELID, 1, read_buf);
				printf("d=0x%02x\n",read_buf[0]);
		...
		if (hex_string[0] == '0' && hex_string[1] == 'x') {
			...
			for (i = 0; i < 32 || i < strlen(hex_string); i++) {
				...
				read_buf[i] = mhexstr2int((char*)hex_buf);
				hex_string = hex_string+2;
			}
		}
		else {
		...
		switch(eep_address_index) {
			case 'a':
				if (!isStr)
					Eeprom_Write(EEP_AD_DATE, 5, (UINT8*)read_buf);
				break;
			case 'b':
				if (!isStr)
					Eeprom_Write(EEP_AD_SERIALCODE_IDENT, 2, (UINT8*)read_buf);
				break;
			case 'c':
				if (isStr)
					Eeprom_Write(EEP_AD_SERIALCODE, SERIAL_CODE_SIZE, (UINT8*)hex_string);
				else
					Eeprom_Write(EEP_AD_SERIALCODE, SERIAL_CODE_SIZE, (UINT8*)read_buf);
				break;
			case 'd':
				if (!isStr)
					Eeprom_Write(EEP_AD_MAC, MAC_SIZE, (UINT8*)read_buf);
				break;
			case 'e':
				if (!isStr)
					Eeprom_Write(EEP_AD_PANELID, 1, (UINT8*)read_buf);
				break;
			default:
				break;
		}
...
int do_eeprom (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if( argc == 1 ) {
		...
		printf("example: eep c \"Tv010: Sep 21 2012  05:01:27  ..\"\n");
		printf("example: eep d 0x001a9a6688ee\n");
		printf("example: eep e 0x11\n\n\n");
	    return OK;
	}

	if( argc >= 2) {
		pch = argv[1];
		if (*pch!='a' && *pch!='b' && *pch!='c' && *pch!='d'&&*pch!='e') {
...
int rtk_plat_do_etherfix(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	printf("Ether Fix for Tv005 L4300 10/100 Ether Phy\n");
...
U_BOOT_CMD(
	...
	"Ether Fix for Tv005 L4300 10/100 Ether Phy",
	"\n"
        "    - Run Ether Fix for Tv005 L4300 10/100 Ether Phy"
);
...
#ifdef CONFIG_CUSTOMIZE_FUNC_REGISTER // SoftDD rev0
#include <common.h>
#include <watchdog.h>
int count_pwm_led = 0;
int skip_condition= 32; // 32ms
...
#else // SoftDD rev0 -> rev2
int count_pwm_led = 0;
void tv005_update_led(void)
{
	if (count_pwm_led == 0)
		IO_PWM_SetDuty(PIN_GREEN_LED, 0xFF);
	count_pwm_led = 1;
}

void tv005_finish_led(void)
{
        int j, T_scale=100; // 100ms
	ulong L_dim_G[14] = { 0xF3, 0xE6, 0xD9, 0xCC, 0xC0, 0xB3, 0xA6, 0x99, 0x8C, 0x80, 0x73, 0x66, 0x59, 0x4C };

        IO_PWM_SetDuty(PIN_RED_LED, 0);
        for (j=0; j < 14; j++) // total about 100ms x 13 = 1.3 seconds
        {
            IO_PWM_SetDuty(PIN_GREEN_LED, L_dim_G[j]);
            mdelay(T_scale);
        }
}
...
int rtk_plat_do_sdd(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	else if (argv[1][0] == 'r' && argv[1][1] == 'g' )
	{
                if ( simple_strtoul(argv[2], NULL, 16) == 0 )    // all off
                {
                        IO_PWM_SetDuty(PIN_GREEN_LED, 0);
                        IO_PWM_SetDuty(PIN_RED_LED, 0);
                }
                else if ( simple_strtoul(argv[2], NULL, 16) == 1 )    // GREEN on; RED off
                {
                        IO_PWM_SetDuty(PIN_GREEN_LED, L_max_G);
                        IO_PWM_SetDuty(PIN_RED_LED, 0);
                }
                else if ( simple_strtoul(argv[2], NULL, 16) == 2 )    // RED on; GREEN off
                {
                        IO_PWM_SetDuty(PIN_GREEN_LED, 0);
                        IO_PWM_SetDuty(PIN_RED_LED, L_max_R);
                }
                else if ( simple_strtoul(argv[2], NULL, 16) == 3 )    // RED on; GREEN on
                {
                        IO_PWM_SetDuty(PIN_GREEN_LED, L_max_G);
                        IO_PWM_SetDuty(PIN_RED_LED, L_max_R);
                }
...
int do_ampset (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
        ulong addr, val;
	...
	// Setting of volume Default = FFh
	amp_IO(2, 0x40, addr, val);

        return 0;
}

U_BOOT_CMD(
        ampset ,    3,    0,     do_ampset,
        "AMP I2C config test",
        "N\n"
        "    - run AMP I2C config test"
);
...
U_BOOT_CMD(
        ...
        "Reset DVT demod for Tv005 L4300",
        "\n"
        "    - Reset DVT demod for Tv005 L4300"
);
#endif
