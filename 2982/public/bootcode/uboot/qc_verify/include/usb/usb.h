...
struct usb_endpoint_descriptor {
	...
	// cyhuang (2007/05/16) : To avoid MSIP CPU WORD alignment.
...
struct usb_device {
	...
	struct usb_device_descriptor descriptor __attribute__ ((aligned (512))); /* Device Descriptor */
	struct usb_config_descriptor config __attribute__ ((aligned (512))); /* config descriptor */
...
#if defined(CONFIG_BOOT_USB_EHCI) || defined(CONFIG_BOOT_USB_OHCI)
...
#endif
/* routines */
int usb_start(int); /* initialize the USB Controller */
int usb_stop(int); /* stop the USB Controller */
void usb_init(void);
...
#define dcache_flush_all		sys_dcache_hit_writeback_all
#define dcache_flush_and_invalidate_all	sys_dcache_flush_and_invalidate_all
#else
