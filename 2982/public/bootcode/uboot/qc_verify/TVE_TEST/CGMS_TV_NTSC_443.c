//#######################################################################################################
//																																										#
//  File Name:      TV_NTSC_443.C
//  Description:    NTSC-4.43 initial setting
//
//#######################################################################################################
#include "TVE_reg.h"
#include "qc_verify.h"
...
void TVE_TV_NTSC_443(UINT32 colorbar)
{
    ...
    rtd_outl(TVE_VIDEO_DAC_CTRL_1_reg,
                     TVE_VIDEO_DAC_CTRL_vdac_apow(1)|
                     TVE_VIDEO_DAC_CTRL_vdac_bpow(0)|
                     TVE_VIDEO_DAC_CTRL_vdac_cpow(0)|
                     TVE_VIDEO_DAC_CTRL_powldv_l(1)|
                     TVE_VIDEO_DAC_CTRL_stanby_l(0)|
                     TVE_VIDEO_DAC_CTRL_biasa_regl(5)|
                     TVE_VIDEO_DAC_CTRL_biasb_regl(5)|
    ...
    rtd_outl(TVE_DAC_MUX_reg,
                     ...
                     TVE_DAC_MUX_write_en5(1)|TVE_DAC_MUX_up_ctrl_int0(3)| //2:upsample 4x, 3:x8
                     TVE_DAC_MUX_write_en4(1)|TVE_DAC_MUX_up_ctrl_int1(3)| //2:upsample 4x, 3:x8
    ...
    //Y	gain
	//0xb8018a00
	rtd_outl(TVE_IP_Y_GAIN_reg, TVE_IP_Y_GAIN_y_gain(0x200)); //y_gain=0x300 gain=1.5, y_gain=0x200 gain=1 
    //0xb8018880
    ...
    //SCH phase
    //0xb8018890 
	rtd_outl(TVE_IP_SCTOH_0_reg, TVE_IP_SCTOH_sctoh(0x0));
	//0xb8018894 
	rtd_outl(TVE_IP_SCTOH_1_reg, TVE_IP_SCTOH_sctoh(0x0));
	//0xb8018898 
	rtd_outl(TVE_IP_SCTOH_2_reg, TVE_IP_SCTOH_sctoh(0x0));
	//0xb801889c 
	rtd_outl(TVE_IP_SCTOH_3_reg, TVE_IP_SCTOH_sctoh(0x0));
    //0xb80188a0
    ...
    //sync level
    //0xb80188b4
	rtd_outl(TVE_IP_SYNC_LVL_reg, TVE_IP_SYNC_LVL_sync_lvl(0x4));//0x7));
    ...
    //burst	amplitude
	//0xb80188c8 
	rtd_outl(TVE_IP_NBA_reg, TVE_IP_NBA_nba(0xc8)); //0xc4
    //0xb80188cc 
	rtd_outl(TVE_IP_PBA_reg, TVE_IP_PBA_pba(0x00));
	//0xb80188d0 
	rtd_outl(TVE_IP_C_CTRL_reg, TVE_IP_C_CTRL_t_pal_clpf(0)|
    ...
    //0xb80189cc 
	rtd_outl(TVE_IP_SYNCLPF_GAIN_reg, TVE_IP_SYNCLPF_GAIN_clpf_gain(0x18));//0x14));
    ...
    //sync timing
    //0xb80189d0
	rtd_outl(TVE_IP_SYNC_STRT_reg, TVE_IP_SYNC_STRT_sync_strt(0x0c)); //0xe
	//0xb80189d4
	rtd_outl(TVE_IP_SYNC_END_reg, TVE_IP_SYNC_END_sync_end(0x4b)); //0x4c
	//0xb80189d8
	rtd_outl(TVE_IP_SYNC_SREND_reg, TVE_IP_SYNC_SREND_sync_srend(0x7a)); //0x79
	//0xb80189dc
	rtd_outl(TVE_IP_SYNC_EQEND_reg, TVE_IP_SYNC_EQEND_sync_eqend(0x2b)); //0x2c

    //active video area
	//0xb80189e0 
	rtd_outl(TVE_IP_ACTV_STRT_reg, TVE_IP_ACTV_STRT_actv_strt(0x85)); //0x89
	//0xb80189e4 
	rtd_outl(TVE_IP_ACTV_END_reg, TVE_IP_ACTV_END_actv_end(0xaa)); //0xa4

    //burst	position
	//0xb80189e8 
	rtd_outl(TVE_IP_WBRST_STRT_reg, TVE_IP_WBRST_STRT_wbrst_strt(0x5a)); //0x58
	//0xb80189ec 
	rtd_outl(TVE_IP_NBRST_STRT_reg, TVE_IP_NBRST_STRT_nbrst_strt(0x62)); //0x60
    //0xb80189f0 
	rtd_outl(TVE_IP_NBRST_END_reg, TVE_IP_NBRST_END_nbrst_end(0x84)); //0x82

    //==============================================================================
    // TV Encoder Setting (NTSC, 443)	
    //==============================================================================
    //Sync generator registers
    //==============================================================================
    //0xb8018004
	rtd_outl(TVE_SYNCGEN_I_reg,
	                 TVE_SYNCGEN_I_write_en2(1)|TVE_SYNCGEN_I_v_thr(524)| //0x20c
					 TVE_SYNCGEN_I_write_en1(1)|TVE_SYNCGEN_I_h_thr(857)); //0x359
    //0xb8018008
	rtd_outl(TVE_SYNCGEN_P_reg,
	                 TVE_SYNCGEN_P_write_en3(1)|TVE_SYNCGEN_P_i_p_sel(1)| //1 : interlaced sync-gen for P-counter
					 TVE_SYNCGEN_P_write_en2(1)|TVE_SYNCGEN_P_v_thr(524)| //0x20c
					 TVE_SYNCGEN_P_write_en1(1)|TVE_SYNCGEN_P_h_thr(857)); //0x359
    //0xb801800c
	rtd_outl(TVE_SYNCGEN_HDMI_reg,
					 ...
					 TVE_SYNCGEN_HDMI_write_en2(1)|TVE_SYNCGEN_HDMI_v_thr(524)| //0x20c
					 TVE_SYNCGEN_HDMI_write_en1(1)|TVE_SYNCGEN_HDMI_h_thr(857)); //0x359
	//0xb80180b0
	rtd_outl(TVE_IP_VSYNCPOS_reg,
	                 TVE_IP_VSYNCPOS_write_en2(1)|TVE_IP_VSYNCPOS_ip_vsync_pos1(524)| //0x20c
					 TVE_IP_VSYNCPOS_write_en1(1)|TVE_IP_VSYNCPOS_ip_vsync_pos2(261)); //0x105
	//0xb80180b4
	rtd_outl(TVE_IP_HSYNCPOS_reg, TVE_IP_HSYNCPOS_hthr(793));//857)); //0x359
	...
	rtd_outl(TVE_AV_I_HPOS_reg,
	                 TVE_AV_I_HPOS_write_en2(1)|TVE_AV_I_HPOS_i_h_st(68)|//132)|
					 TVE_AV_I_HPOS_write_en1(1)|TVE_AV_I_HPOS_i_h_end(787));//851)); //(130,849) 

	
    //0xb801804c
    rtd_outl(TVE_SLEW_I_HPOS_reg,
                     TVE_SLEW_I_HPOS_write_en2(1)|TVE_SLEW_I_HPOS_i_h_st(68)|
                     TVE_SLEW_I_HPOS_write_en1(1)|TVE_SLEW_I_HPOS_i_h_end(787));

	//0xb801805c
	rtd_outl(TVE_ITOP_VPOS_reg,
	                 TVE_ITOP_VPOS_write_en2(1)|TVE_ITOP_VPOS_itop_v_st(21)|
					 TVE_ITOP_VPOS_write_en1(1)|TVE_ITOP_VPOS_itop_v_end(260)); //can not larger 261
    //0xb8018060
	rtd_outl(TVE_IBOT_VPOS_reg,
	                 TVE_IBOT_VPOS_write_en2(1)|TVE_IBOT_VPOS_ibot_v_st(284)|
					 TVE_IBOT_VPOS_write_en1(1)|TVE_IBOT_VPOS_ibot_v_end(523)); //(285,524)
	...
	rtd_outl(TVE_AV_P_HPOS1_reg,
					 ...
					 TVE_AV_P_HPOS1_write_en1(1)|TVE_AV_P_HPOS1_p_h_end(856)); //(236,596)
    //0xb8018064
	...
	rtd_outl(TVE_AV_H_HPOS1_reg,
					 ...
					 TVE_AV_H_HPOS1_write_en2(1)|TVE_AV_H_HPOS1_h_h_st(136)|
					 TVE_AV_H_HPOS1_write_en1(1)|TVE_AV_H_HPOS1_h_h_end(856));
    //0xb801806C
	...
	rtd_outl(TVE_I_COLORBAR_HPOS_reg,
					 ...
					 TVE_I_COLORBAR_HPOS_write_en1(1)|TVE_I_COLORBAR_HPOS_i_h_st_pos(0x50));//68));//0x84));
	...
	rtd_outl(TVE_COMP_PBPR_SYNCLVL_reg,
	                 TVE_COMP_PBPR_SYNCLVL_write_en2(1)|TVE_COMP_PBPR_SYNCLVL_pbpr_negsynclvl(0x40)|//0x450
    ...
    rtd_outl(TVE_SYNCGEN_RST_reg,
                     ...
                     TVE_SYNCGEN_RST_write_en3(0)|TVE_SYNCGEN_RST_i_sync_go(1)|
                     TVE_SYNCGEN_RST_write_en2(0)|TVE_SYNCGEN_RST_p_sync_go(1)|
                     TVE_SYNCGEN_RST_write_en1(0)|TVE_SYNCGEN_RST_h_sync_go(1));	
