...
//check after
#define KSEG1ADDR
extern void qc_dev_print (block_dev_desc_t *dev_desc);
extern int qc_get_partition_info (block_dev_desc_t *dev_desc, int part, disk_partition_t *info);
...
fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	static unsigned char buffer[SECTOR_SIZE] __attribute__ ((aligned (512)));
#else
	...
	if (dev_desc->block_read (dev_desc->dev, 0, 1, (unsigned long *) buffer) != 1) {
		printk ("** Can't read from device %d **\n", dev_desc->dev);
		return -1;
	}
...
*/
		...
		if(!qc_get_partition_info(dev_desc, part_no, &info)) {
			part_offset = info.start;
			cur_part = part_no;

//--			printk("part_offset=%x  cur_part=%x\n",part_offset,cur_part);
		}
		else {
			printk ("** Partition %d not valid on device %d **\n",part_no,dev_desc->dev);
			return -1;
		}
// #else
...
get_cluster(fsdata *mydata, __u32 clustnum, __u8 *buffer, unsigned long size)
{
	...
	/*add by fox USB download file again*/
	/*modify by fox for USB download file size is not enough over 512 issue */
	...
	if(size % FS_BLOCK_SIZE) {
		static __u8 tmpbuf[FS_BLOCK_SIZE];//  __attribute__ ((aligned (512)));
	     ...
	     unsigned long maxsize)
{
	...
	FAT_DPRINT("Filesize: %d bytes, mydata = 0x%x\n", filesize ,mydata );

	if (maxsize > 0 && filesize > maxsize) filesize = maxsize;

	FAT_DPRINT("Reading: %d bytes\n", filesize);
...
#if defined(CONFIG_USB_PATCH)
__u8	 get_vfatname_block[MAX_CLUSTSIZE] __attribute__ ((aligned (512)));
#else
 ...
 */
#if defined(CONFIG_USB_PATCH)
__u8 get_dentfromdir_block[MAX_CLUSTSIZE] __attribute__ ((aligned (512)));
#else
...
{
#if defined(CONFIG_USB_PATCH)
	static __u8 block[FS_BLOCK_SIZE] __attribute__ ((aligned (512)));
#else
...
#if defined(CONFIG_USB_PATCH)
__u8 do_fat_read_block[MAX_CLUSTSIZE] __attribute__ ((aligned (512))); /* Block buffer */
#else
...
#if 0
    fsdata *mydata = (fsdata *) KSEG1ADDR(&datablock);
#else
...
*/
    ...
    FAT_DPRINT ("FAT%d, fatlength: %d\n", mydata->fatsize,
		mydata->fatlength);
    FAT_DPRINT ("Rootdir begins at sector: %d, offset: 0x%x, size: %d\n"
		"Data begins at: %d\n",
...
file_fat_detectfs(void)
{
	...
	if(cur_dev==NULL) {
		printk("No current device\n");
		return 1;
	}
#if 1 /*(CONFIG_COMMANDS & CFG_CMD_IDE) || (CONFIG_COMMANDS & CFG_CMD_SCSI) || (CONFIG_COMMANDS & CFG_CMD_USB) || (CONFIG_MMC)*/
	printk("Interface:  ");
	switch(cur_dev->if_type) {
		case IF_TYPE_IDE :	printk("IDE"); break;
		case IF_TYPE_SCSI :	printk("SCSI"); break;
		case IF_TYPE_ATAPI :	printk("ATAPI"); break;
		case IF_TYPE_USB :	printk("USB"); break;
		case IF_TYPE_DOC :	printk("DOC"); break;
		case IF_TYPE_MMC :	printk("MMC"); break;
		default :		printk("Unknown");
	}
	printk("\n  Device %d: ",cur_dev->dev);
	qc_dev_print(cur_dev);
#endif
	...
	vol_label[11] = '\0';
	volinfo.fs_type[5]='\0';
	printk("Partition %d: Filesystem: %s \"%s\"\n",cur_part,volinfo.fs_type,vol_label);
	return 0;
}
