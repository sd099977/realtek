...
extern int qc_test_part_dos (block_dev_desc_t *dev_desc);
...
extern int qc_get_partition_info_dos (block_dev_desc_t *dev_desc, int part, disk_partition_t * info);
extern void qc_print_part_dos (block_dev_desc_t *dev_desc);
/*
 ...
 */
void qc_dev_print (block_dev_desc_t *dev_desc)
{
#ifdef CONFIG_LBA48
	...
	if (dev_desc->if_type==IF_TYPE_SCSI)  {
		printk ("(%d:%d) ", dev_desc->target,dev_desc->lun);
	}
	...
	switch (dev_desc->type & 0x1F) {
		...
		case DEV_TYPE_TAPE: 	puts ("Tape");
					break;
		default:		printk ("# %02X #", dev_desc->type & 0x1F);
					break;
	}
	puts ("\n");
	if ((dev_desc->lba * dev_desc->blksz)>0L) {
		...
		printk ("            Capacity: %lu.%lu MB = %lu.%lu GB (%lu x %lu)\n",
			mb_quot, mb_rem,
			gb_quot, gb_rem,
			lba,
			dev_desc->blksz);
...
void qc_init_part (block_dev_desc_t * dev_desc)
{
	...
	if (qc_test_part_dos(dev_desc) == 0) {
...
int qc_get_partition_info (block_dev_desc_t *dev_desc, int part, disk_partition_t *info)
{
...
#ifdef CONFIG_DOS_PARTITION
	case PART_TYPE_DOS:
		if (qc_get_partition_info_dos(dev_desc,part,info) == 0) {
...
void qc_print_part (block_dev_desc_t * dev_desc)
{
