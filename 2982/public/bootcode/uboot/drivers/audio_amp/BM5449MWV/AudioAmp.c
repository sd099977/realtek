...
void AudioAmp_InitAudioOut2I2S_2nd(void)
{
	...
	if ( m_I2C_MASTER != 0 && m_AMP_DEVICE_ADDR != 0 )
	{
		...
		/* The cautions at the time of starting
		   Please be sure to send the following command after the IC reset release containing the power supply ON.
		   Release power down mode(PDX = H)
		   Please wait for about 20ms until PLL is stabilized.
		   1．&hE9[7:0] = 34h : The system clock inside IC is set up.
		   2．&h03[5:0] = 02h : I2S 24bit   BCLK = 64fs
		   3．&h01[7:0] = 00h : RAM clear OFF
		   4．Set to "TV Initial Data" of other register
		*/
		amp_IO(m_I2C_MASTER, m_AMP_DEVICE_ADDR, 0xE9, 0x34);
        	...
        	//amp_IO(m_I2C_MASTER, m_AMP_DEVICE_ADDR, 0x0C, 0x00); // Not used
        	amp_IO(m_I2C_MASTER, m_AMP_DEVICE_ADDR, 0x0D, 0x02);
        	amp_IO(m_I2C_MASTER, m_AMP_DEVICE_ADDR, 0x0E, 0x02);
        	//amp_IO(m_I2C_MASTER, m_AMP_DEVICE_ADDR, 0x0F, 0x00); // Not used
