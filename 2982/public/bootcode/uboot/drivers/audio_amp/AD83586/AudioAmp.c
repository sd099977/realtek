...
void AudioAmp_InitAudioOut2I2S_1st(void)
{
  ...
  if (m_PIN_AMP_MUTE) {
    ...
    IO_Set(m_PIN_AMP_MUTE, 0);
		printf("get m_PIN_AMP_MUTE = %llu (reset AMP!)\n", m_PIN_AMP_MUTE);
  }

	#else
	m_I2C_MASTER = PCB_I2C_3;
...
{	
	...
	if(i2c_master < PCB_I2C_3)	{
		...
		i2c_print("G2C_Write_EX!!!!!\n");
		i2c_print("xAudioAmp_Write =>chipAddr=%02x, regLen=%d, regval=%02x, Data=%02x\n", (unsigned char)m_AMP_DEVICE_ADDR, 1, addr[0], data[0]);		
		if (G2C_Write_EX(i2c_master, (unsigned char)m_AMP_DEVICE_ADDR, 1, addr, w_len-1, data) != S_OK) {
			return R_ERR_RX_ERROR;
		}
	}
...
unsigned long xAudioAmp_Read(UINT8 i2c_master, UINT8 w_len, UINT8 r_len, UINT8* write_buf, UINT8* read_buf)
{
	...
	if(i2c_master < PCB_I2C_3)	{
		i2c_print("I2C_Read_EX!!!!!\n");
		if (I2C_Read_EX(i2c_master, m_AMP_DEVICE_ADDR, w_len, write_buf, r_len, read_buf) != S_OK) {
			return R_ERR_RX_ERROR;
		}
	}
	else {
		i2c_print("G2C_Read_EX!!!!!\n");
		i2c_print("xAudioAmp_Read =>chipAddr=%02x, regLen=%d, regval=%08x, rDataLen=%d\n", (unsigned char)m_AMP_DEVICE_ADDR, 1, write_buf[0], w_len);
		if (G2C_Read_EX(i2c_master, (unsigned char)m_AMP_DEVICE_ADDR, w_len, write_buf, r_len, read_buf) != S_OK) {
			return R_ERR_RX_ERROR;
		}
	}
	mdelay(10); //use HW i2c need to add this to avoid set error
