...
//use rbus/iso_reg.h & rbus/mis_reg.h
...
#define GPIO_DIR						(0xB801B104)
#define GPIO_DATO						(0xB801B120)
#define GPIO_DATI						(0xB801B13c)
#define SDA_BIT							(22)
#define SCL_BIT							(21)
#else
...
/*
 *add for TCL 2982, 	allen_li		2014/4/16
 */
//G2C Port 2
#define G2C_PORT2_PIN_MUX						(0xB8060208)
#define G2C_PORT2_PIN_MUX_MASK				((0x1F << 27) | (0x1F << 18))
#define G2C_PORT2_PIN_MUX_ENABLE			((0x1F << 27) | (0x1F << 18))
//MISC GPIO Port 2
#define PORT2_GPIO_DIR						(0xb8061100)
#define PORT2_GPIO_DATO					(0xb8061108)
#define PORT2_GPIO_DATI					(0xB801BC40)
#define PORT2_SDA_BIT						(21)
#define PORT2_SCL_BIT						(20)
...
#define G2C_DELAY 						for(ii=0; ii<6; ii++) { }		//100KHz before enable_caches()
#else
#define G2C_DELAY 						for(ii=0; ii<280; ii++) { }		//100KHz
#endif

//#define G2C_DELAY 						udelay(2)  the most is 43KHz ...?
...
static int g2c_sync(unsigned char id)
{
	...
	{
		PIN_MUX_REG			= G2C_PORT2_PIN_MUX;
		PIN_MUX_MASK		= G2C_PORT2_PIN_MUX_MASK;
		PIN_MUX_ENABLE	= G2C_PORT2_PIN_MUX_ENABLE;

		SDA_DIR_REG			= PORT2_GPIO_DIR;
		SDA_DATO_REG		= PORT2_GPIO_DATO;
		SDA_DATI_REG		= PORT2_GPIO_DATO;
		SDA_PIN					= PORT2_SDA_BIT;

		SCL_DIR_REG			= PORT2_GPIO_DIR;
		SCL_DATO_REG		= PORT2_GPIO_DATO;
		SCL_DATI_REG		= PORT2_GPIO_DATO;
		SCL_PIN					= PORT2_SCL_BIT;
	}
    ...
    else if(id == 4)
    {
        ...
        PIN_MUX_MASK             = 0;
        PIN_MUX_ENABLE          = 0;
        SDA_DIR_REG             = 0xb8061104;//get_GPDIR(PIN_I2C2_SDA);
         SDA_DATO_REG             = 0xb806110c;//get_GPO(PIN_I2C2_SDA);
         SDA_DATI_REG               = 0xb8061114;//get_GPI(PIN_I2C2_SDA);
         SDA_PIN                          = 3;

         SCL_DIR_REG                  = 0xb8061100;//get_GPDIR(PIN_I2C2_SCL);
         SCL_DATO_REG              = 0xb8061108;//get_GPO(PIN_I2C2_SCL);
         SCL_DATI_REG                = 0xb8061110;//get_GPI(PIN_I2C2_SCL);
         SCL_PIN                           = 25;
    }
#endif
...
void G2C_Init(unsigned char Id)
{
	...
	printf("pin_mux_reg[0x%08x] = 0x%08x\n", 0xb8060234, GET_G2C_PINMUX(0xb8060234));
	printf("pin_mux_reg[0x%08x] = 0x%08x\n", 0xb8060240, GET_G2C_PINMUX(0xb8060240));
#if defined(CONFIG_BOARD_298X_TV010_QFP176_V3_EMMC)

#else
  if(Id == 4){
		// Panel TCON control : SDA_PIN: st_gpio35 , SCL : st_gpio25
        rtd_maskl(0xb8060234, ~(0x1f<<27), 0x1f<<27);
        rtd_maskl(0xb8060240, ~(0x1f<<18), 0x1f<<18);
    }
#endif
	printf("pin_mux_reg[0x%08x] = 0x%08x\n", 0xb8060234, GET_G2C_PINMUX(0xb8060234));
	printf("pin_mux_reg[0x%08x] = 0x%08x\n", 0xb8060240, GET_G2C_PINMUX(0xb8060240));
