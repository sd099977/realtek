...
extern int mmc_data_sync( struct mmc * mmc );
...
int mmc_startup(struct mmc *mmc)
{
    ...
    /* enter pullpush state, set clock big then 400kHz */
    mmc_set_clock(mmc, 400001);
...
#ifdef CONFIG_BSP_REALTEK // just decrease the initialized time
	mmc->part_num = 0;
	err = TIMEOUT;
#else
