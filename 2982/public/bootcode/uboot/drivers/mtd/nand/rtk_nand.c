...
static void nf_set_random_read(unsigned int pagesize,unsigned int spare_col)
{
	...
	rtd_outl(REG_RND_SPR_STR_COL_H, (spare_col+ pagesize) >> 8);	// spare start address MSB
	rtd_outl(REG_RND_SPR_STR_COL_L, (spare_col+ pagesize) & 0xff);	// spare start address LSB
}
