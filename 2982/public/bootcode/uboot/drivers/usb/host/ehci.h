...
#define	ehci_readl(x)		(*((volatile u32 *)GET_MAPPED_RBUS_ADDR((u32)x)))
#define ehci_writel(a, b)	(*((volatile u32 *)GET_MAPPED_RBUS_ADDR((u32)a)) = ((volatile u32)b))
				
#else
#if 0
#define ehci_readl(x)		cpu_to_le32((*((volatile u32 *)GET_MAPPED_RBUS_ADDR((u32)x))))
#define ehci_writel(a, b)	(*((volatile u32 *)GET_MAPPED_RBUS_ADDR((u32)a)) = \
					cpu_to_le32(((volatile u32)b)))
#else
static inline unsigned int ehci_readl(uint32_t volatile *regs)
{
	rmb();
	return rtd_inl((unsigned int)regs);
}

static inline void ehci_writel(uint32_t volatile *regs, const unsigned int val)
{
	wmb();
