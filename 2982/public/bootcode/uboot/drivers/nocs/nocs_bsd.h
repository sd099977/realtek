...
**
**  $Id: //CAK/components/bsdapi/MAIN/src/nocs_bsd.h#8 $
**  $Change: 150124 $
**
** REMARK:
...
#define BSDAPI_VERSION_MEDIUM 4
#define BSDAPI_VERSION_MINOR  3
 ...
 *
 *  - <b> 3.4.3 - 19-Dec-2012 </b>
 *   - Clarified usage of SHA and HMAC context
 *   - Clarified management of buffers and forced usage of bsdAllocateBuffer()
 *     for RAM2RAM and digest functions.
 *   - Removed unwanted const qualifier on bsdFreeBuffer()
 *   - Removed constraint on RAM2RAM input message size (no longer needs to
 *     be a multiple of cipher block size).
 *
 *  - <b> 3.4.2 - 17-Oct-2012 </b>
 *    - Restricted NOCS1 API profile to strict minimum
 *
 *  - <b> 3.4.0 - 28-Sep-2012 </b>
 *    - Added bsdAllocateBuffer() and bsdFreeBuffer() to comply with DMA
 *      buffer constraints.
 *    - Added support of RAM2RAM in-place operations
 *    - Added #NAGRA_IPR_NOTICE_SIZE
 *
 *  - <b> 3.3.0 - 28-Jun-2012 </b>
 *    - Added NOCS1.x specific functions bsdEncryptDataWithSecretContentKey()
 *      and bsdDecryptDataWithSecretContentKey().
 *
 *  - <b> 3.2.4 - 31-May-2012 </b>
 ...
 *  depends on the version of NOCS. Refer to \ref p_api_profile "NOCS API profiles"
 ...
 *  calculations. BSD API functions use byte array buffers for passing in and
 ...
 *    work properly. bsdAllocateBuffer() and bsdFreeBuffer() are designed to
 *    handle such buffers.
 *
 *    Although many BSD API functions parameters are defined as byte array
 *    buffers, it has been decided to force the usage of bsdAllocateBuffer() only
 ...
 *
 *    These considerations do not apply to bsdEncryptDataWithSecretContentKey()
 *    and bsdDecryptDataWithSecretContentKey() functions, because of backward
 *    compatibility with older client applications.
*/
 ...
 *    copied from input to output buffer.
 *
 ...
 *
 *  @pre
 *    The content key has been set through bsdSetClearTextKey(), bsdSet2LevelProtectedKey(),
 *    bsdSetSecretContentKey() or bsdUseFlashProtKey().
 *
 *  @param[in]  xSession
 *    Handle of the session
 *
 ...
 *    bsdAllocateBuffer() or a similar function that ensures compatibility with
 ...
 *    required by CBC. This cipher mode of operation is part of the EMI definition.
 *    If no IV is required (ECB mode), pxInitVector is set to NULL.\n
 *
 ...
 *    function to achieve operations requiring high performances.
*/
 ...
 *    - \ref p_buffer
*/
TUnsignedInt8* bsdAllocateBuffer
(
  size_t  xBufferSize
);
 ...
 *
 *  @brief
 *    This function frees a buffer allocated by bsdAllocateBuffer().
 *
 *  @applies
 *    NOCS3 only
 *
 ...
 *
 *  @note
 *    -# The context allocation is handled by the driver.
 ...
 *      context and return #BSD_ERROR_INVALID_PARAMETERS.
 */
 ...
 *  @retval  BSD_ERROR_INVALID_PARAMETERS
 *    An invalid parameter is provided:
 *    - NULL key (\c xKeySize=NULL) with a non-null key size (xKeySize>0). Note
 *      that an null key size is not considered as an error, whatever the value
 *      of \c pxKey.
 *    - Invalid key size (xKeySize>16)
