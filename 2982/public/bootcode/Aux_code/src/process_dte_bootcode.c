...
static int verify_dte_bootcode(const unsigned int img_addr, const int secure_type)
{
	...
	rom_copy_memory(&g_dte_bootcode_tail, (char *)(_img_addr + bootcode_bin_size - 4), sizeof(DTE_BOOTCODE_AREA_Tail));
	...
	switch (secure_type) {
		...
		case DCAS_SECURE_BOOT:
			...
			rom_copy_memory((char *)(tmp + bootcode_bin_size), (char *)(_img_addr + bootcode_bin_size), sizeof(DTE_BOOTCODE_AREA_Tail) - 4);
...
int process_dte_bootcode(const int flash_type, const int secure_type)
{
	...
	unsigned int tmp = 0;
#endif
	unsigned int dst_addr = KSEG1(rom_params->Address_of_Boot_Code_Area);
	...
	g_dte_bootcode_area_size = rom_params->External_Boot_Specific_Params + sizeof(DTE_BOOTCODE_AREA_Tail);
