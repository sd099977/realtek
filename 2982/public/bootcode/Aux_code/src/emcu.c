...
int download_8051_imem(const int flash_type)
{
    ...
    unsigned int pagesize = *rom_s_PageSize;
    unsigned int blocksize = *rom_s_BlockSize;
    unsigned int pageperblock = *rom_s_PagePerBlock;
    ...
    switch (flash_type) {
        ...
        case NAND_FLASH_BOOT:
            ...
            if( rom_nf_read_image(EMMC_SRAM_BUF,
        ...
        case EMMC_BOOT:
            ...
            if(rom_mmc_blk_read(EMMC_EMCU_BLK,
                                IMAGE_SIZE_BLK,
    ...
    switch (flash_type) {
        ...
        case NAND_FLASH_BOOT:
            ...
            if( rom_nf_read_image(EMMC_SRAM_BUF_WOW,
                                &sys_flash_current_pos,
        ...
        case EMMC_BOOT:
            ...
            REG32(0xb801084c) &= ~0x8;  // switch Dbus to DDR
            if( rom_mmc_blk_read( EMMC_EMCU_BLK_WOW,
                                    IMAGE_SIZE_WOW_BLK,
...
#define CLKEN_EMCU2_CORE	(0x01UL<<1)
    ...
    for (i=0; i<(IMAGE_SIZE_WOW/4); i++){
        value = (((unsigned int) buf[3 + i*4 + IMAGE_SIZE])<<24) |
                (((unsigned int) buf[2 + i*4 + IMAGE_SIZE])<<16) |
                (((unsigned int) buf[1 + i*4 + IMAGE_SIZE])<< 8) |
                buf[0+ i*4 + IMAGE_SIZE];
