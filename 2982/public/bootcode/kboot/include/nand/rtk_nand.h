...
#define REG_MASK_U32(offset, andMask, orMask)  rtd_outl(offset, ((rtd_inl(offset) & (andMask)) | (orMask)))
#endif
...
//for init secure store area

#define DEFAULT_SECURE_STORE_BLOCK_SIZE 0x300000	// 3M
#define DEFAULT_SECURE_STORE_BLOCK_RES_LEN  0xED800  //0x100000;

typedef struct{
	unsigned int secure_store_saddr;
	unsigned int secure_store_len;
	unsigned int secure_store_block_size;

	unsigned int secure_store_rw_saddr;
	unsigned int secure_store_rw_len;

	unsigned int secure_store_ro_saddr;
	unsigned int secure_store_ro_len;
}secure_store_info;

typedef struct ss_posix_header_t
{                   /* byte offset */
    char name[100]; /* 0 */
    char mode[8]; /* 100 */
    char uid[8]; /* 108 */
...
typedef struct ss_posix_header_bk_t
{                   /* byte offset */
    char name[100]; /* 0 */
    char mode[8]; /* 100 */
    char uid[8]; /* 108 */
    ...
    char devminor[8]; /* 337 */
    char prefix[155]; /* 345 */
    unsigned int rtk_filecrc32; /*500 */     
    unsigned int rtk_tarsize; /*504 */     	
    char rtk_signature[4]; /*508 */         	
} ss_posix_header_bk;

extern unsigned int nand_crc32(unsigned char *p, int len, unsigned int *crc);
...
#define RTK_FLUSH_CACHE(addr, len) 	dcache_flush_invalidate()
#endif
...
#ifdef CONFIG_MTD_NAND_IBT
	int (*read_oob_ibt) (struct mtd_info *mtd, u16 chipnr, int page, int len, u_char *oob_buf);
#endif
	int		eccmode;
	int		eccsize;
	int		eccbytes;
...
#ifdef CONFIG_MTD_NAND_IBT
	unsigned char bbt_err_status;		//status: B1_ERR,B2_ERR,BOTH_ERR
	IB_t *ibt;			//ibt
	unsigned char use_ibt;		//use ibt flag
	unsigned char ibt_err_status;		//status: B1_ERR,B2_ERR,BOTH_ERR
