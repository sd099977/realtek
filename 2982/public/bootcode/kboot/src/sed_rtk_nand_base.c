...
static int rtk_strcmp(const char* s1, const char* s2)
{
	while (*s1 || *s2)	
	{
	 if (*s1!= *s2)
...
static int rtk_secure_store_ro_data_check(struct mtd_info *mtd, u16 chipnr, secure_store_info *ss_info)
{
	...
	if(rtk_strcmp(p_header->rtk_signature,"RTK")){
		printf("[Warning][%d] Check secure_store header failed!\n",__LINE__);
		return -1;
	}
		...
		for (i = 0; nand_device[i].name; i++){
			...
			if ( nand_device[i].id==nand_type_id &&
				...
				if (nand_device[i].size == num_chips_probe * nand_device[i].chipsize){
					...
					if ( !rtk_strcmp(nand_device[i].name, "HY27UF084G2M" ) )
						continue;
					else{
...
int TEST_READ_WRITE(struct mtd_info *mtd, int page)
{
	...
	//this->g_databuf = KSEG1ADDR(this->g_databuf);
	//this->g_oobbuf = KSEG1ADDR(this->g_oobbuf);
	...
	this->g_databuf[12] = 0xcc;
	this->g_databuf[13] = 0xbb;
	this->g_databuf[14] = 0xee;
	...
	this->g_oobbuf[4] = 0xaa;

	printf("g_databuf = 0x%x  g_oobbuf = 0x%x \n", this->g_databuf, this->g_oobbuf);
