...
#define sed_printf0(...)    printf(__VA_ARGS__)
#define sed_printf1(...)    printf1(__VA_ARGS__)
#else
#define sed_printf(...)     do{}while(0)
#define sed_printf1(...)    do{}while(0)
#endif
...
#define BUF_LEN             0x10000                     //64k bytes

#define TOP_BUF_START       0x80600000                  //64k bytes
#define TOP_CMD_BUF         TOP_BUF_START               //64k bytes
#define TOP_RSP_BUF         (TOP_CMD_BUF+BUF_LEN)       //64k bytes
#define RSA_MEM_POOL		(TOP_RSP_BUF+BUF_LEN)       //64k bytes
#define OTP_POOL			(RSA_MEM_POOL+BUF_LEN)      //64k bytes
#define OTP_BUF_ADR         (OTP_POOL+BUF_LEN)          //64k bytes
#define TDES_IDATA_BUF      (OTP_BUF_ADR    +BUF_LEN)   //64k bytes
#define TDES_ODATA_BUF      (TDES_IDATA_BUF +BUF_LEN)   //64k bytes
#define AES_IDATA_BUF       (TDES_ODATA_BUF +BUF_LEN)   //64k bytes
#define AES_ODATA_BUF       (AES_IDATA_BUF  +BUF_LEN)   //64k bytes
#define RSA_IDATA_BUF       (AES_ODATA_BUF  +BUF_LEN)   //64k bytes
#define RSA_ODATA_BUF       (RSA_IDATA_BUF  +BUF_LEN)   //64k bytes
#define TDES_KEY_BUF        (RSA_ODATA_BUF  +BUF_LEN)   //16 bytes
#define AES_KEY_BUF         (TDES_KEY_BUF   +KEY_LEN)   //16 bytes
#define RSA_KEY_BUF         (AES_KEY_BUF    +KEY_LEN)   //16 bytes
#define NEXT_BUF            (RSA_KEY_BUF    +KEY_LEN)   //16 bytes
