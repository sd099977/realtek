...
//-----------------------------------------//
static int get_nand_boot_type(int *ecc_type, int *access_mode, int *page_size)
{
...
#if 1
	printf("\n-----------------pinmux value----------------------\n");
	printf("0xb8000830 = 0x%x \n",REG32(0xb8000830));
	printf("0xb8000834 = 0x%x \n",REG32(0xb8000834));
	printf("0xb8000838 = 0x%x \n",REG32(0xb8000838));
	printf("0xb800083c = 0x%x \n",REG32(0xb800083c));
	printf("0xb8000840 = 0x%x \n",REG32(0xb8000840));
	printf("0xb8000844 = 0x%x \n",REG32(0xb8000844));
	printf("0xb8000038 = 0x%x \n",REG32(0xb8000038));
		...
		else
		{
			if (verify_after_write) {
				...
				if ((temp != spare) || compare_memory(data_ptr, (UINT8 *)PAGE_TMP_ADDR, device->PageSize)) {
					...
					page_counter -= (current_page % pages_per_block);
					current_page = current_block_no*pages_per_block;
					printf("current_page = 0x%x page_counter 0x%x data_ptr = 0x%x data_len=%d\n", current_page,page_counter, (UINT32)data_ptr,data_len);
					continue;
#endif
				}
			}
...
int do_identify_n(void **dev)
{
	...
	UINT8 id5, id6;
	int ecc_type,access_mode, p_size;
	...
	printf("ecc bit = 0x%x\n", ((n_device_type *)(*dev))->ecc_bit);
	
	// set random access mode
	get_nand_boot_type(&ecc_type, &access_mode, &p_size);
	if(access_mode == 0x1){
...
#if defined(Config_Nand_Erase_Flash_ALL)
	*(UINT8 *)(0x1000000) = 0x23;
	printf("force erase entire flash\n");
#endif
	...
	if(*(UINT8 *)(0x1000000) == 0x23) {
		...
		for (i = 0; i < /*blk_state_len*/ nand_bootcode_area_size / device->BlockSize; i++)
		{
			...
			switch (state)
			{
				...
				case BLOCK_BOOTCODE:
				case BLOCK_KBOOT:
#ifdef SKIP_PROGRAM_BOOTCODE /* don't erase bootcode, if skip program bootcode is set */
					break;
#endif					
...
void write_bbt(void *dev)
{
	...
	len *= device->PageSize;
	sys_dcache_flush((unsigned long)PAGE_TMP_ADDR, len);
...
static int build_bbt(void *dev)
{
	...
	// check for bad block
	for (i = /*nand_bootcode_area_size / device->BlockSize*/0, bbt_i = 0; 
