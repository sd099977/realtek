...
static int romcr_wait_opt_end(UINT8 cmdcode,UINT32 cmd_type)
{
    ...
    while(timeend )
    {
        ...
        {
            if((sd_stat_reg & DAT0_LEVEL) == 0){
                mmcinfo("DAT0_LEVEL is LOW!!\n");
            }else{
                ...
                {
                    printf("sd_transfer_reg(08h)=0x%02x[bit6]\n", sd_transfer_reg);
                    err = -1;
                }
                else if( sd_int_stat_reg & RTKCR_INT_DECODE_ERROR ||
                         sd_stat_reg & MASK_SD_STAT_ERR )
                {
                    printf("sd_stat(40h)   =0x%02x(error bit[7:5])\n", sd_stat_reg);
                    printf("int_status(48h)=0x%02x(decode error[bit2])\n", sd_int_stat_reg);
                    err = -2;
                }else{
                    err = 0;
                }
...
static void romcr_set_clk(UINT8 mmc_clk)
{
    ...
    tmp_clk = cr_readl(SYS_CLKSEL) & ~EMMC_CLKSEL_MASK;

    printf("emmc: clk change to %s",
            clk_tlb[mmc_clk>>EMMC_CLKSHT_BIT]);
                                   ...
                                   UINT32 *rsp)
{
    ...
    err = romcr_wait_opt_end(SD_SENDCMDGETRSP,INN_CMD);
               ...
               const unsigned int page_offset)
{
    ...
    if(!mmc_card_blockaddr(card)){
        ...
        mmcinfo("sector addressing card\n");
        tar_addr = *dest;
    }
    blk_cont = rem_size>>9;
    if(rem_size & 0x1ff)
...
int do_identify_e(void **dev)
{
    ...
    *dev = &emmc_card;
    printf("eMMC bootcode burning tool 2014/03/17 22:00\n");
    printf("(build at %s %s )\n",__DATE__,__TIME__);
    ...
    //make for FPGA
    romcr_set_clk(CARD_SWITCHCLOCK_16MHZ);
...
#define MMC_SECTOR_ADDR  			(0x40000000UL)
    ...
    while (timeend--){
        ...
        if (rom_resp[0] & MMC_CARD_BUSY){
            if( rom_resp[0] & MMC_SECTOR_ADDR ) {
                mmc_card_set_blockaddr(&emmc_card);
    ...
    /* speed up to 20MHz */
    //mmcinfo("set clock 20MHz\n");
    //romcr_set_clk(CARD_SWITCHCLOCK_20MHZ);
