		...
		if (m_dTotalColumns != 0)
		{
			...
			while (!m_rSheet->IsEOF())
			{
				...
				try
				{
					...
					m_stempString.Empty();
					//m_stempString.GetBufferSetLength(4096);
					...
					//	m_stempSql.Replace('\n', ' ');
						...
						if (column != m_dTotalColumns-1) // Not last column
						{
							CString tmp;
							char buffer[4096];
							sprintf(buffer, "%s\"%s\"%s", (LPCTSTR) m_stempString, (LPCTSTR) m_stempSql, (LPCTSTR) m_sSeparator);
							tmp.Format("%s\"%s\"%s", (LPCTSTR) m_stempString, (LPCTSTR) m_stempSql, (LPCTSTR) m_sSeparator);
							CString length;
							length.Format("%d %d %d %d %d", tmp.GetLength(), m_stempString.GetLength(), m_stempSql.GetLength(), m_stempSql.GetLength(), strlen((LPCTSTR) m_stempSql));
							
							m_stempString = buffer;
							length.Format("%d %d %d", m_stempString.GetLength(), strlen((LPCTSTR)m_stempString), strlen(buffer));
						}
						else // Last column
						{	
							m_stempString = m_stempString + "\"" + m_stempSql + "\"";
