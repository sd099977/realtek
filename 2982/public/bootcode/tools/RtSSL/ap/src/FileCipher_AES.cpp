...
{           
    ...
    if (flen < 16 && pad_algo==NO_PADDING)        
        dbg_print("Info Enc/Dec - input data less than 16 Bytes(%d) without padding, these data will remain clear\n", flen);            
    ...
    {                       
        if (flen > BuffLen && (flen - BuffLen) > BlockSize)
            DataLen = fread(Buff, 1, BuffLen-16, fd1);
        else            
            DataLen = fread(Buff, 1, BuffLen, fd1);
...
int AES_FileEncryption(ENC_CMD* cmd)
{
    ...
    case CIPHER_ALGO_AES_128_ECB:             
        return AES_ECB_FileEncryption(cmd->op_code, key, cmd->sfile, cmd->dfile, GetPaddingAlgo(cmd->padding_str));
    
    case CIPHER_ALGO_AES_128_CBC:            
        return AES_CBC_FileEncryption(cmd->op_code, key, iv, cmd->sfile, cmd->dfile, GetPaddingAlgo(cmd->padding_str));        
    }
