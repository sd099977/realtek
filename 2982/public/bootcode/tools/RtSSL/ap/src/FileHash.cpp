...
#define STRING_SFILE_STRIP	".sfile.strip"
#define STRING_HASH_IN		".hash_in"
#define STRING_HASH_OUT		".hash_out"
#define HASH_TMP1			".hash1.tmp"
#define HASH_TMP2			".hash2.tmp"

const char string_sfile_strip[16]	= STRING_SFILE_STRIP;
const char string_hash_in[16]		= STRING_HASH_IN;
const char string_hash_out[16]		= STRING_HASH_OUT;
const char string_hash_1[16]		= HASH_TMP1;
const char string_hash_2[16]		= HASH_TMP2;
    ...
    )
{
    ...
    {
        if (StripNandImgae(cmd->sfile, STRING_SFILE_STRIP, &cmd->fmt.nand)<0)
        {
            printf("Generate hash value failed - strip Nand image file failed\n");
            return -1;
        }

        cmd->sfile = (char *)string_sfile_strip;
    }
...
int SignHashValue(SIGN_ALGO* sign, unsigned char* hash, unsigned int hash_len)
{
    ...
    if (sign->algo_str)
    {
        ...
        enc.sfile    = (char *)string_hash_in;
        enc.dfile    = (char *)string_hash_out;
        enc.key_str  = sign->key_str;
        enc.iv_str   = sign->iv_str;

        MemoryToFile(STRING_HASH_IN, hash, hash_len);

        EncryptFile(&enc);
        hash_len = GetFileLength(STRING_HASH_OUT);
        ReadDataFromFile(STRING_HASH_OUT, hash, hash_len, 0);
    }
