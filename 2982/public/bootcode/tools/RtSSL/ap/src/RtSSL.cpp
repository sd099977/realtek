...
const char string_stdin[16]			= STRING_STDIN;
const char string_stdout[16]		= STRING_STDOUT;
const char string_hash_out[16]		= HASH_OUT_TEMP;
const char string_sha256_out[16]	= SHA256_OUT_TEMP;
...
int DoCipherCommand(ENC_CMD* cmd)
{
    ...
    case CIPHER_DECRYPTION:
        if (strcmp(cmd->sfile, STRING_STDIN)==0)
            DumpDataFromStdin(cmd->sfile);

        ret = EncryptFile(cmd);

        if (ret==0 && strcmp(cmd->dfile, STRING_STDOUT)==0)
...
int ParseHashCommand(int argc, char *argv[], CMD_STR* cmd)
{
    ...
    hash->sfile    = (char *)string_stdin;
    hash->dfile    = (char *)string_stdout;
...
int DoHashCommand(HASH_CMD* cmd)
{
    ...
    case DIGEST_APPEND_HASH_VALUE:
		...
		if( cmd->append_sha256 ) {
			...
			cmd->dfile = (char *)string_sha256_out;
        ...
        cmd->dfile = (char *)string_hash_out;
        GenerateHashValue(cmd);
        ...
        case DIGEST_GENERATE_HASH_VALUE:
            ...
            if ((cmd->vmode & HASH_OUT_IN_ASCII) || strcmp(target_file, STRING_STDOUT)==0)
            ...
            if (strcmp(target_file, STRING_STDOUT)==0)
