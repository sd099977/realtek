...
if(defined $ARGV[2])
{
	open (CONFIG_FILE10, "$ARGV[2]");
	print "Input RSA Info File : "  . substr( $ARGV[2], rindex($ARGV[2], "/") + 1) . "\n";
}

if(defined $ARGV[3])
{
	open (CONFIG_FILE11, "$ARGV[3]");
	print "Input RSA R2 Info File : "  . substr( $ARGV[3], rindex($ARGV[3], "/") + 1) . "\n";
}
